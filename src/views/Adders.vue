      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item,index) in addersListFilter" :class="{'check':checkIndex===index}"
...
<script>
  ...
  export default {
    ...
    methods: {
      getAdders() {
        axios.get('/goods/adderslist').then(res => {
          if (res.status === 200 && res.data.code === 0) {
            this.addersList = res.data.result.addressList
          }
        });
      },
      moreshow() {
        if (this.limit < this.addersList.length) {
          this.limit = this.addersList.length
        } else {
          this.limit = 3;
        }
      },
    ...
    computed: {
      addersListFilter() {
        return this.addersList.slice(0, this.limit)
      }
    }
  }
</script>
